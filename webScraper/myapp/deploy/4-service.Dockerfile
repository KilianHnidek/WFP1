...
COPY /myapp/deploy/4-service.requirements.txt /4-service.requirements.txt

RUN /venv/bin/pip install --disable-pip-version-check --no-cache-dir -r /4-service.requirements.txt
...
ENTRYPOINT ["/venv/bin/python3", "backend.py"]
